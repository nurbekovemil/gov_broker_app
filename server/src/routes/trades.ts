...
import { calcBrokerMargin } from '../services/pricing';
...
router.post('/', authenticate, async (req, res) => {
  ...
  try {
    ...
    const brokerMargin = calcBrokerMargin(quantity, parseFloat(p.dirty_price));

    // For sell: check portfolio
    if (tradeType === 'sell') {
      const portRes = await client.query(
        'SELECT quantity FROM portfolio WHERE user_id=$1 AND bond_id=$2',
        [userId, bondId],
      );
    ...
    if (tradeType === 'buy') {
      ...
      await client.query(
        'DELETE FROM portfolio WHERE user_id=$1 AND bond_id=$2 AND quantity<=0',
        [userId, bondId],
      );
    }
