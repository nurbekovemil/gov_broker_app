...
router.get('/margin', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
...
router.get('/balances', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    const result = await pool_1.pool.query(`
    SELECT
      u.full_name, u.email,
      b.isin, b.name AS bond_name,
...
router.get('/summary', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    const [trades, margin, clients, bonds] = await Promise.all([
        pool_1.pool.query(`SELECT COUNT(*) AS count, SUM(total_amount) AS volume FROM trades WHERE created_at::date = CURRENT_DATE`),
        pool_1.pool.query(`SELECT COALESCE(SUM(broker_margin),0) AS total FROM trades`),
        pool_1.pool.query(`SELECT COUNT(DISTINCT user_id) AS count FROM portfolio WHERE quantity>0`),
        pool_1.pool.query(`SELECT COUNT(*) AS count FROM bonds WHERE status='active'`),
    ]);
    return res.json({
